...
export class AgnesError extends Error {
  ...
  /** Only definite, key-scoped provider rejection permits submission rotation. */
...
/** A documented queue-full response contains no accepted-task receipt and is retry-safe. */
export function isAgnesProviderCapacityRejection(payload: unknown): boolean {
  ...
  return /\b(?:(?:video(?: generation)?|render|request)\s+)?queue\s+(?:is\s+)?(?:full|at capacity)\b/.test(text)
    ...
    || /\b(?:agnes|provider|render(?:ing)?|video(?: generation)?|server|service|system)\s+(?:is\s+)?(?:temporarily\s+)?(?:busy|overloaded)\b/.test(text)
    || /\b(?:temporarily\s+)?(?:busy|overloaded)\s+(?:server|service|system)\b/.test(text);
}
...
export interface ClassifyAgnesErrorOptions {
  ...
  keys?: readonly string[];
  retryAfterMs?: number;
  /** A syntactically successful POST without a receipt may have been accepted. */
...
export function classifyAgnesError(
  ...
  // A 503 carrying the provider's explicit queue/busy rejection is definite:
  // no key rotation is useful and the scene should remain pending next run.
  // Other server failures remain ambiguous even if untrusted prose happens to
  // contain words such as "quota" or "rate".
  if (isAgnesProviderCapacityRejection(payload) && (status < 500 || status === 503)) {
    return new AgnesError(message, { ...base, kind: "provider_capacity" });
  }
  if (status >= 500) {
    return new AgnesError(message, { ...base, kind: "provider", ambiguousOutcome: true });
  }
  // A syntactically successful POST without a valid receipt may already have
  // created a task. Do not infer rate/quota text from that ambiguous body and
  // rotate to another key, which could create a second render immediately.
  ...
  if (status === 429 || isRateLimited(diagnostics)) {
