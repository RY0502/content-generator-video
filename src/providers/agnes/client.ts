...
export const DEFAULT_AGNES_SUBMISSION_MAX_RETRIES = 3;
export const DEFAULT_AGNES_SUBMISSION_RETRY_INTERVAL_MS = 60_000;
...
function normalizeTask(
  ...
  // The documentation uses metadata.url. The live Flash endpoint has also
  // returned url at the task root, so normalize both observed shapes.
...
function requiredPrompt(value: unknown): string {
  ...
  // Agnes does not document a prompt-size limit. Do not silently truncate the
  // caller's scene/continuity prompt and change its meaning.
  return prompt;
}

function validDuration(value: number): number {
  if (!Number.isInteger(value) || value < AGNES_MIN_SECONDS || value > AGNES_MAX_SECONDS) {
    throw new AgnesError(
      `seconds must be an integer from ${AGNES_MIN_SECONDS} to ${AGNES_MAX_SECONDS}`,
      { kind: "validation" },
    );
  }
  return value;
...
function validSeed(value: number | undefined): number | undefined {
  if (value === undefined) return undefined;
  if (!Number.isSafeInteger(value)) {
    throw new AgnesError("seed must be a safe integer", { kind: "validation" });
  }
  return value;
}
...
function validReferenceImageUrl(value: unknown, index: number): string {
  ...
  if (
    ...
    throw new AgnesError(`images[${index}] must be publicly reachable by Agnes`, {
      kind: "validation",
    });
  }
  return candidate;
...
function validReferenceImages(request: AgnesSubmitVideoRequest): string[] | undefined {
  const runtimeRequest = request as AgnesSubmitVideoRequest & { images?: unknown };
  if (request.mode === "text") {
    if (runtimeRequest.images !== undefined) {
  ...
  if (request.mode !== "reference") {
    throw new AgnesError("mode must be text or reference", { kind: "validation" });
  }
  if (!Array.isArray(runtimeRequest.images) || runtimeRequest.images.length < 1) {
    throw new AgnesError("reference mode requires at least one image URL", {
      kind: "validation",
    });
  }
  if (runtimeRequest.images.length > AGNES_MAX_REFERENCE_IMAGES) {
    throw new AgnesError(
      `reference mode accepts at most ${AGNES_MAX_REFERENCE_IMAGES} image URLs`,
      { kind: "validation" },
    );
  }
  return runtimeRequest.images.map(validReferenceImageUrl);
}
...
function resolveAgnesEndpoints(baseUrl: string): AgnesEndpoints {
  ...
  retrieveUrl.pathname = createPath || "/v1/videos";
  return {
...
/** Return the canonical completed output, rejecting aliases and non-HTTPS URLs. */
export function completedAgnesVideoUrl(task: AgnesVideoTask): string {
  ...
  if (!candidate) {
    throw new AgnesError("Completed Agnes task is missing its output URL", {
      kind: "provider",
    });
  }
  let parsed: URL;
  try {
    ...
    throw new AgnesError("Completed Agnes task has an invalid output URL", {
      kind: "provider",
    });
  }
...
export class AgnesVideoClient {
  ...
  private readonly submissionMaxRetries: number;
  private readonly submissionRetryIntervalMs: number;
  ...
  constructor(options: AgnesClientOptions = {}) {
    ...
    this.submissionMaxRetries = finiteMilliseconds(
      "submissionMaxRetries",
      options.submissionMaxRetries ?? DEFAULT_AGNES_SUBMISSION_MAX_RETRIES,
      true,
    );
    this.submissionRetryIntervalMs = finiteMilliseconds(
      "submissionRetryIntervalMs",
      options.submissionRetryIntervalMs ?? DEFAULT_AGNES_SUBMISSION_RETRY_INTERVAL_MS,
      true,
    );
  }
  ...
  /** Submit exactly one text or image-reference Flash task. */
  async submitVideo(request: AgnesSubmitVideoRequest): Promise<AgnesVideoTask> {
    ...
    const seed = validSeed(request.seed);
    ...
    const payload = {
      ...
      aspect_ratio: AGNES_VIDEO_ASPECT_RATIO,
      n: 1,
      ...(seed === undefined ? {} : { seed }),
    ...
    for (let index = 0; index < this.keys.length; index += 1) {
      ...
      while (true) {
        try {
          ...
          // Diagnostics callbacks must never influence provider behavior.
        }
        ...
        if (response.ok && !result.malformedJson) {
          try {
            ...
            if (task.status === "failed" && task.error !== undefined) {
              ...
              if (failedTaskError.mayTryAnotherKey && keyAttempt < this.submissionMaxRetries) {
                keyAttempt += 1;
                const delayMs = Math.max(this.submissionRetryIntervalMs, failedTaskError.retryAfterMs ?? 0);
                if (delayMs > 0) await this.sleep(delayMs);
                continue;
              }
              if (failedTaskError.mayTryAnotherKey && index + 1 < this.keys.length) break;
            ...
            if (classified.mayTryAnotherKey && index + 1 < this.keys.length) break;
        ...
        if (classified.mayTryAnotherKey && keyAttempt < this.submissionMaxRetries) {
          keyAttempt += 1;
          const delayMs = Math.max(this.submissionRetryIntervalMs, classified.retryAfterMs ?? 0);
          if (delayMs > 0) await this.sleep(delayMs);
          continue;
        }
        if (classified.mayTryAnotherKey && index + 1 < this.keys.length) break;
  ...
  /** Retrieve with the exact submitting key selected by its persisted fingerprint. */
  async retrieveVideo(task: AgnesVideoTask): Promise<AgnesVideoTask> {
    ...
    const endpointBase = this.endpoints.retrieveVideo.replace(/\/+$/, "");
    const url = new URL(`${endpointBase}/${encodeURIComponent(task.video_id.trim())}`);
    let response: Response;
    let result: ReadPayloadResult;
    try {
  ...
  /** Poll immediately, then at the configured interval for one bounded window. */
  async pollUntilTerminal(
    ...
    while (firstPoll || Math.max(0, now() - startedAt, scheduledElapsedMs) <= window) {
      ...
      try {
        ...
        // A completed task is usable only once the signed output capability is present.
        ...
        const transient = error instanceof AgnesError
          && ["timeout", "network", "provider", "provider_capacity", "rate_limit"]
            .includes(error.kind);
  ...
  /** Stream the signed media URL to a temporary file, then publish it atomically. */
  async downloadCompletedVideo(
    ...
    let response: Response;
    try {
      // The output URL is a media capability, not an Agnes API endpoint.
