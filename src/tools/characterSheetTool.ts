  ...
  return new DynamicStructuredTool({
    ...
    func: async ({ seriesId }) => {
      ...
      return JSON.stringify({
        ...
        nextAction:
          "Continue the exact resumeAction returned by get_next_episode. For first-time authoring, the next assistant action must be write_episode_script_chunk(operation=start): put the complete plan strictly matching the episode title and premise and only opening scenes 1-8 of the continuous 16-scene arc in tool arguments (do not conclude in scenes 1-8); emit no visible planning, manual counting, draft, JSON, or preamble.",
      });
    },
  });
}
